...
mutable struct MockLinQuadModel # <: LinQuadOptInterface.LinQuadOptimizer
    ...
    function MockLinQuadOptimizer(env::Env,str::String="defaultname")
        m = new()
        ...
        m.primal_status = m.FeasiblePoint
        m.dual_status = m.FeasiblePoint
...
LQOI.LinearQuadraticModel(::Type{MockLinQuadOptimizer},env) = MockLinQuadModel(env::Env,"defaultname")
...
LQOI.get_linear_dual_solution!(instance::MockLinQuadOptimizer, place)
    # Fix QUADRATICS
    for i in eachindex(place)
